...
async def download_file(url: str, target_folder: Path):
    ...
    async with semaphore:  # Limit the concurrency
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(url, follow_redirects=True)  # Follow redirects
...
if __name__ == "__main__":
    ...
    parser.add_argument("lower_bound", type=int, help="The lower bound of episode numbers")
    parser.add_argument("upper_bound", type=int, help="The upper bound of episode numbers")
    parser.add_argument("target_folder", type=str, help="The folder to save downloaded files")
